return {
	...
	keys = {
		...
		},
		{
			"<leader>fa",
			function()
				require("telescope.builtin").live_grep({ additional_args = { "--glob", "!fakeout/**" } })
			end,
			desc = "Live grep (exclude fakeout)",
		},
		{
			"<leader>fB",
		...
		{
			"<leader>fF",
			':lua require("telescope.builtin").grep_string({ search = "FIXME: (JMT)" })<CR>',
			desc = "Grep FIXME (JMT)",
		},
		{
			"<leader>fb",
			':lua require("telescope.builtin").grep_string({ search = "BOOKMARK:" })<CR>',
			desc = "Grep BOOKMARK:",
		},
	},
	config = function()
